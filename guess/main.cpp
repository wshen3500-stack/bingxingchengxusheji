...
int main()
{
    ...
    while (!q.priority.empty())
    {
        ...
        if (curr_num > 1000000)
        {
            ...
            int total_guesses = q.guesses.size();
            int batch_count = 0;
            string batch_inputs[4];
            bit32 batch_states[4][4];

            for (int i = 0; i < total_guesses; i++)
            {
                // TODO：对于SIMD实验，将这里替换成你的SIMD MD5函数
                batch_inputs[batch_count] = q.guesses[i];
                batch_count++;
                
                if (batch_count == 4) {
                    MD5Hash_SIMD(batch_inputs, batch_states);
            ...
            for (int k = 0; k < batch_count; ++k) {
                ...
                MD5Hash(batch_inputs[k], single_state); 
            }
            // 在这里对哈希所需的总时长进行计算
